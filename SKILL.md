...
description: "Turn any codebase into a beautiful, interactive single-page HTML course that teaches how the code works to non-technical people. Use this skill whenever someone wants to create an interactive course, tutorial, or educational walkthrough from a codebase or project. Also trigger when users mention 'turn this into a course,' 'explain this codebase interactively,' 'teach this code,' 'interactive tutorial from code,' 'codebase walkthrough,' 'learn from this codebase,' 'make a course from this project,' '把这个项目做成课程', '解读这个代码库', '帮我理解这段代码', '把代码库变成课程', '生成交互式教程'. This skill produces a stunning, self-contained HTML file with scroll-based navigation, animated visualizations, embedded quizzes, and code-with-plain-English side-by-side translations."
---
...
**所有生成的课程内容必须使用简体中文（Simplified Chinese）。** 这包括：
- 所有模块标题、小节标题、正文段落
- 测验题目、选项、答题反馈解释
- 悬浮词汇提示（tooltip）的定义文字
- 代码对照翻译块中的"白话文"一栏
- 标注、说明框（Callout）内容
- 导航栏中的模块名称

**唯一保持英文的内容：** 代码块左侧显示的原始代码（必须与源文件完全一致，不做任何修改）。

This applies to every course generated by this skill, regardless of the language the user makes the request in.

## 感知型与目录型内容的可视化原则 / Visual Showcase Rule

在分析代码库后，识别其中的**感知型内容**和**目录型内容**，并为它们创建独立的可视化展示模块——不能仅靠文字描述。

**感知型内容（Perceptual Content）：** 颜色、字体、间距、阴影、动画、视觉层次等。这类内容的理解依赖眼睛而非大脑——`#D94F30` 这串字符传递不了任何信息，但色块一秒就够了。设计系统（design system）、主题令牌（theme tokens）、视觉规范文件均属此类。

**目录型内容（Catalog Content）：** 组件库、选项清单、模式库、API 端点集合等完整枚举型内容。光列名字毫无意义——必须逐一渲染，让读者建立「这个工具箱里有什么」的真实感知。

**当课程遇到这类内容时，必须：**
1. 为每类创建独立的展示模块（不与普通概念模块混排）
2. 逐一可视化渲染：颜色令牌显示色块、字体令牌显示字体样本、组件逐个实例化展示
3. 尽可能使用交互方式（可点击、可切换、可悬停）而不是静态截图或代码片段
4. 在「工具概览」模块的相关卡片正下方，加入预告段落和跳转按钮，引导读者前往对应展示模块

**判断标准：** 如果某内容用文字描述后，读者仍然无法形成直觉性理解，它就属于需要可视化展示的内容。

## 图标规范：禁止 Emoji，使用语义化内联 SVG / Icon Rule

**严禁在课程 HTML 中使用 Unicode Emoji**（📖🎨🧰💡⚠🤖 等）作为图标。Emoji 有强烈的 AI 生成感，视觉风格与课程的精致设计系统不兼容。

**所有图标必须使用内联 SVG**，遵循以下规范：
- 风格：Lucide Icons 风格——细线（stroke-width: 1.5）、圆角端点（round）、无填充为主
- 颜色：使用 `stroke="currentColor"`，随上下文颜色自动适配，禁止硬编码颜色值
- 尺寸：按使用场景匹配——pattern-card-icon 用 26×26，callout-icon 用 22×22，flow-actor-icon 用 22×22，file-tree 用 16×16，状态标签用 13×13
- 语义：每个图标必须与其代表的概念有直接视觉语义关联（大脑→思维、调色盘→设计、工具箱→工具集），不得随意套用不相关图形
- 格式：`<svg width="N" height="N" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle">...</svg>`

这条规则适用于课程中所有视觉图标场景：pattern cards、callout boxes、flow animation actors、file trees、status tags。
